...
grep -q '"slug": "gpt-6-astra"' "$models_cache" || fail "gpt-6-astra is not in the Codex model cache"

model_block="$(sed -n '/"slug": "gpt-6-astra"/,/"shell_type"/p' "$models_cache")"
grep -q '"effort": "xhigh"' <<<"$model_block" || fail "gpt-6-astra does not advertise xhigh reasoning"

printf 'Codex: %s\n' "$codex_version"
printf 'Grok: %s\n' "$grok_version"
printf 'Orchestrator contract: gpt-6-astra / xhigh\n'
